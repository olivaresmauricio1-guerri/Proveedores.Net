...
Public Class frmProveedoresSelector
    ...
    Private Sub GridBuscar()
        ...
        Dim sql As String = "SELECT * FROM MaeCtaCte WHERE 1=1"
        Dim parametros As New List(Of Object) From {"@Sucursal", General.SucursalActual}

        If Not String.IsNullOrEmpty(texto) Then
            sql &= " AND (NroCuenta LIKE @NroCuenta OR Nombre LIKE @Nombre OR Cuit LIKE @Cuit)"
            parametros.AddRange(New Object() {"@NroCuenta", $"%{texto}%", "@Nombre", $"%{texto.Trim()}%", "@Cuit", $"%{texto.Trim()}%"})
        End If

        Dim tabla = DSM.ExecuteQuery(DSM.Proveedores, sql, CmdParams(parametros.ToArray()))
    ...
    Public Sub GridConfigurarColumnas()
        ...
        DgvListado.Columns("NroCuenta").DisplayIndex = 0
